...
class LibtorrentFlutter {
  ...
  static void _onAlert(
    ...
    // Alerts are consumed so the native queue cannot grow indefinitely.
    // Do not decode or print them here: high-seed torrents can emit thousands
    // of tracker, peer, and DHT alerts per minute on the UI isolate. Playback
    // state and actionable errors are available through the public status APIs.
  }
  ...
  /// Clean up a single torrent: stops its streams and removes it.
  ///
  /// Returns true if the torrent existed and was removed, false if it was
  /// already gone.
  bool disposeTorrent(int torrentId, {bool deleteFiles = false}) {
    ...
    removeTorrent(torrentId, deleteFiles: deleteFiles);
    return true;
  }

  /// Clean up ALL torrents. Call with [deleteFiles] only for an explicit
  /// storage-clear action; normal app shutdown preserves playback cache.
  void disposeAll({bool deleteFiles = false}) {
    ...
    // Remove all torrents, retaining their data by default.
    for (final tid in _torrents.keys.toList()) {
      try {
        removeTorrent(tid, deleteFiles: deleteFiles);
      } catch (_) {}
    }
  }
  ...
  Future<void> dispose({bool deleteFiles = false}) async {
    disposeAll(deleteFiles: deleteFiles);
    _pollTimer?.cancel();
